...
def load_detector_geometry(geofile_path):
    ...
    def process_subsystem(parent_path):
        ...
        for i in range(pvol.GetNdaughters()):
            ...
            label, color, alpha = None, None, 0.3
            if 'Wall' in name and 'border' not in name:
                label, color, alpha = 'Target Emulsion Wall (Grey)', ROOT.kGray+1, 0.45
            elif 'ScifiVolume' in name:
                label, color, alpha = 'SciFi Station (Blue)', ROOT.kAzure+7, 0.5
            elif 'subVetoBox' in name or 'volVetoPlane' in name:
                label, color, alpha = 'Veto Detector (Orange)', ROOT.kOrange+7, 0.45
            elif 'FeBlock' in name:
                label, color, alpha = 'MuFilter Iron Block (Dark Green)', ROOT.kGreen+3, 0.35
            elif 'subUSBox' in name or 'subDSBox' in name:
                label, color, alpha = 'MuFilter Active Plane (Blue)', ROOT.kBlue-4, 0.5
...
def main():
    ...
    for i_event, event in enumerate(tree):
        ...
        if (primary_idx != -1 or inducing_idx != -1) and len(my_daughters) >= 2:
            ...
            dir_name = f"Event_{evt_num}_Zvert{int(z_vert_val)}cm"
            event_dir = f_out.mkdir(dir_name)
            if not event_dir:
                event_dir = f_out.Get(dir_name)
            ROOT.SetOwnership(event_dir, False)
            event_dir.cd()
            ...
            mg_xz.SetTitle(f"XZ Projection (Event #{evt_num}, Z_v={z_vert_val:.1f}cm);Z [cm];X [cm]")
            ...
            banner_xz.DrawLatex(0.12, 0.92, f"#bf{{SND@LHC Trident}}  Event #{evt_num}  (Entry {i_event})")
            ...
            mg_yz.SetTitle(f"YZ Projection (Event #{evt_num}, Z_v={z_vert_val:.1f}cm);Z [cm];Y [cm]")
            ...
            banner_yz.DrawLatex(0.12, 0.92, f"#bf{{SND@LHC Trident}}  Event #{evt_num}  (Entry {i_event})")
            ...
            h3 = ROOT.TH3F(f"box_{evt_num}_Idx_{i_event}", f"3D View (Event #{evt_num}, Z_v={z_vert_val:.1f}cm);Z [cm];X [cm];Y [cm]",
                           10, Z_3D_MIN, Z_MAX,
            ...
            x_p_3d, y_p_3d, z_p_3d = get_trajectory_endpoints(primary_track, Z_3D_MIN, Z_MAX)
            ...
            if is_secondary_inducing:
                ind_z_start_3d = max(z_ind[0], Z_3D_MIN)
                pl_ind = ROOT.TPolyLine3D(2)
                ROOT.SetOwnership(pl_ind, False)
                pl_ind.SetPoint(0, ind_z_start_3d, x_ind[0], y_ind[0])
                pl_ind.SetPoint(1, z_ind[1], x_ind[1], y_ind[1])
