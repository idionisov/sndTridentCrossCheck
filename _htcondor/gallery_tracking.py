...
def get_args():
    ...
    parser.add_argument('-par', '--parFile', type=str, default="/afs/cern.ch/user/i/idioniso/snd_master/sndsw/trackingParams.xml", help='Tracking parameter file')
...
def main():
    ...
    conf = ROOT.snd.Configuration(
        ROOT.snd.Configuration.GetOption(daq_run),
        geo.modules['Scifi'],
        geo.modules['MuFilter']
    )

    with open(OUT_CSV, mode='a', newline='') as csv_file:
        fieldnames = ['run', 'event_number', 'shower_start', 'n_tracks', 'sum_hit_weight_density', 'sum_qdc', 'max_ang_diff']
        ...
        for i_entry in range(start_ev, start_ev + n_break):
            ...
            scifi_planes = ROOT.snd.analysis_tools.FillScifi(conf, tree.Digi_ScifiHits, geo.modules['Scifi'])
            us_planes = ROOT.snd.analysis_tools.FillUS(conf, tree.Digi_MuFilterHits, geo.modules['MuFilter'])
            sh_start = ROOT.snd.analysis_tools.GetScifiShowerStart(scifi_planes)
            ...
            for ht_trk in sf_task.kalman_tracks:
                # 1. Get the track state (position and momentum)
                # We use the fitted state from the first measurement point
                try:
                    # '0' indicates the first hit/state in the track
                    state = ht_trk.getFittedState()
                    mom = state.getMom() # Returns a TVector3
                    
                    # 2. Calculate slopes (equivalent to your old getAngleXZ)
                    # slope = dx/dz = Px / Pz
                    curr_xz = mom.X() / mom.Z()
                    curr_yz = mom.Y() / mom.Z()
                except:
                    curr_xz = prev_slopes[0]
                    curr_yz = prev_sloeps[1]

                if prev_slopes is None:
                    prev_slopes = (curr_xz, curr_yz)
                    continue
                
                # 3. Calculate the differences
                ang_diff_xz = abs(curr_xz - prev_slopes[0])
                ang_diff_yz = abs(curr_yz - prev_slopes[1])
                
                ang_diff = max(ang_diff_xz, ang_diff_yz)
                if max_ang_diff < ang_diff:
            ...
            writer.writerow({
                ...
                'shower_start': sh_start,
                'n_tracks': n_tracks,
                'sum_hit_weight_density': sum_density,
                'sum_qdc': qdc,
                'max_ang_diff': 1e3*max_ang_diff
            })

            if n_tracks > 2:
                print(f"Match:\n\tRun:\t{daq_run}\n\tEvent:\t{event_number}\n\tnTracks:/t{n_tracks}\n\tHit weight density:\t{sum_density}\n\tSciFi QDC:\t{qdc}\n\tMax Ang Diff:\t{1e3*max_ang_diff}\n\tShower tag:\t{sh_start!=-1}")
