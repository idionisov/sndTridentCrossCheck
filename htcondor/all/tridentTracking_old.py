...
def getHoughLines(ht_task, event, geo, z_vtx_max=200):
    ...
    for proj_name in ['XZ', 'YZ']:
        ...
        while valid_lines_found < ht_task.max_reco_muons and attempts < max_attempts:
            ...
            if SndlhcMuonReco.numPlanesHit(hit_collection['system'][m][hits_rel], hit_collection['detectorID'][m][hits_rel]) >= ht_task.min_planes_hit:
                ...
                close_mask = np.any(dist_sq < 0.03, axis=1)
    ...
    if counts['XZ'] > counts['YZ']:
        return counts['XZ'], max_dangle_XZ, lines

    elif counts['YZ'] > counts['XZ']:
        return counts['YZ'], max_dangle_YZ, lines

    else:
        return counts['XZ'], max(max_dangle_XZ, max_dangle_YZ), lines



def get_lines_vtx(lines, z_det_max=200):
    """
    Gets the intersection point of Hough Lines.
    Real tridents are highly boosted and originate far upstream, 
    so they should not cross inside or near the detector.
    
    Returns:
        float: z_vtx (float or None))
    """
    z_vtx = None
    
    for proj in ['XZ', 'YZ']:
        lines_proj = lines[proj]

        if len(lines_proj) < 2:
            continue
            
        m1, c1 = lines_proj[0][0], lines_proj[0][1]
        m2, c2 = lines_proj[1][0], lines_proj[1][1]
...
def main():
    ...
    with open(
        ...
        for i_entry in range(args.start_event, args.start_event + n_break):
            ...
            n_tracks, max_dangle, lines = getHoughLines(sf_task, tree_in, geo, z_vtx_max=0)
            if n_tracks >= 2:
