...
def main():
    ...
    with open(
        ...
        df = pd.read_csv("/afs/cern.ch/work/i/idioniso/sndTridentValidation/htcondor/gallery/gallery_events.csv")
        whitelist = set(zip(df['run'], df['event_number']))
        ...
        for i_entry in range(args.start_event, args.start_event + n_break):
            ...
            if whitelist is not None and (daq_run, event_number) not in whitelist:
                continue
