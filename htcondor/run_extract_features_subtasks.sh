#!/usr/bin/env bash
# ==============================================================================
# SND@LHC Trident BDT Feature Extractor - Subtask Runner
# ==============================================================================
# Usage examples:
#   1) Run only the first 10,000 events:
#      ./run_extract_features_subtasks.sh -n 10000
#
#   2) Run a specific subtask chunk (e.g. chunk index 0 -> events 0 to 10,000):
#      ./run_extract_features_subtasks.sh --task-id 0 --chunk-size 10000
#
#   3) Run all chunks (e.g., 50k events in 5 parallel subtasks of 10k each):
#      ./run_extract_features_subtasks.sh --task-id all --chunk-size 10000 -j 4 --merge output/merged.parquet
#
#   4) Custom input file and geofile:
#      ./run_extract_features_subtasks.sh \
...
PYTHON_EXTRACTOR=""
for cand in \
    "${SCRIPT_DIR}/extract_trident_bdt_features.py" \
    "${SCRIPT_DIR}/../extract_trident_bdt_features.py" \
    "/afs/cern.ch/work/i/idioniso/sndMuTri/extract_trident_bdt_features.py" \
    "$(pwd)/extract_trident_bdt_features.py"; do
    if [[ -f "${cand}" ]]; then
        PYTHON_EXTRACTOR="$(cd "$(dirname "${cand}")" && pwd)/$(basename "${cand}")"
        break
    fi
done
...
for cand in \
    ...
    "${SCRIPT_DIR}/../TrackingParams_sf4.xml" \
    "/afs/cern.ch/work/i/idioniso/sndMuTri/TrackingParams_sf4.xml" \
    "$(pwd)/TrackingParams_sf4.xml"; do
    if [[ -f "${cand}" ]]; then
...
DEFAULT_OUTDIR="${SCRIPT_DIR}/extracted_features"
...
CHUNK_SIZE=10000
TASK_ID=""             # empty = single run with FIRST_EVENT / MAX_EVENTS, integer = specific chunk, "all" = all chunks
START_EVENT=0
...
Options:
  ...
  -e, --extractor PATH     Python feature extractor script (Default: ${PYTHON_EXTRACTOR})
  -o, --outdir DIR         Output directory for Parquet files (Default: ${DEFAULT_OUTDIR})
  ...
  -m, --merge PATH         Optional destination path to concatenate all chunk Parquet files into one
...
Examples:
  ...
  # Run all 10k event chunks in parallel and merge:
  ./$(basename "$0") -t all -c 10000 -j 4 --merge "${OUT_DIR}/master.parquet"
EOF
    exit 0
}
...
if [[ $# -gt 0 && ! "$1" =~ ^- ]]; then
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -e|--extractor) PYTHON_EXTRACTOR="$2"; shift 2 ;;
            -o|--outdir|--output)
...
if [[ -z "${PYTHON_EXTRACTOR}" || ! -f "${PYTHON_EXTRACTOR}" ]]; then
    echo "[-] Error: Feature extractor script not found at '${PYTHON_EXTRACTOR}'"
    exit 1
fi

if [[ ! -f "${PAR_FILE}" ]]; then
    for cand in "${SCRIPT_DIR}/${PAR_FILE}" "${SCRIPT_DIR}/../${PAR_FILE}" "/afs/cern.ch/work/i/idioniso/sndMuTri/${PAR_FILE}" "$(pwd)/${PAR_FILE}"; do
        if [[ -f "${cand}" ]]; then
...
# Helper function to execute extraction on a single chunk
run_chunk_job() {
    local in_f="$1"
    local start_ev="$2"
    local n_ev="$3"
    ...
    echo "  [Subtask] Start: ${start_ev} | Events: ${n_ev} -> $(basename "${out_parquet}")"
    python3 "${PYTHON_EXTRACTOR}" \
        -i "${in_f}" \
        -o "${out_parquet}" \
        -g "${GEO_FILE}" \
        -p "${PAR_FILE}" \
...
echo "========================================================================"
echo "SND@LHC Feature Extraction Subtask Launcher"
echo "========================================================================"
...
if [[ -n "${TASK_ID}" && "${TASK_ID}" =~ ^[0-9]+$ ]]; then
    ...
    N_EV="${CHUNK_SIZE}"
    OUT_PARQUET="${OUT_DIR}/${BASE_NAME}_chunk$(printf "%04d" "${TASK_ID}").parquet"
    
    echo "[+] Running Subtask #${TASK_ID} (Events ${START} to $(( START + N_EV )))..."
    run_chunk_job "${INPUT_FILE}" "${START}" "${N_EV}" "${OUT_PARQUET}"
...
# Mode B: Run all chunks
    ...
    for (( i=0; i<TOTAL_CHUNKS; i++ )); do
        ...
        OUT_PARQUET="${OUT_DIR}/${BASE_NAME}_chunk$(printf "%04d" "${i}").parquet"
        CHUNK_FILES+=("${OUT_PARQUET}")
    ...
    if [[ -n "${MERGE_OUTPUT}" ]]; then
        echo "[+] Merging subtask parquet files into '${MERGE_OUTPUT}'..."
        python3 -c "
import pandas as pd, glob, sys, os
files = [f for f in sys.argv[1:] if os.path.exists(f)]
if files:
    df = pd.concat([pd.read_parquet(f) for f in files], ignore_index=True)
    df.to_parquet('${MERGE_OUTPUT}', engine='pyarrow', compression='snappy', index=False)
    print(f'  Merged {len(df):,} total rows -> ${MERGE_OUTPUT}')
" "${CHUNK_FILES[@]}"
    fi
...
# Mode C: Direct Start Event / N Events (e.g. first 10,000 events)
    ...
    N_EV="${N_EVENTS}"
    if [[ -n "${EXPLICIT_OUTFILE}" ]]; then
        OUT_PARQUET="${EXPLICIT_OUTFILE}"
    elif [[ "${N_EV}" -eq 0 ]]; then
        OUT_PARQUET="${OUT_DIR}/${BASE_NAME}_all.parquet"
    else
        OUT_PARQUET="${OUT_DIR}/${BASE_NAME}_ev${START}_to_$(( START + N_EV )).parquet"
    fi

    echo "[+] Running extraction for Events ${START} to $(( START + N_EV ))..."
    run_chunk_job "${INPUT_FILE}" "${START}" "${N_EV}" "${OUT_PARQUET}"
fi
