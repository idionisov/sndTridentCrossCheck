...
if not hasattr(ROOT, "computeSciFiMLMetrics"):
    ...
    double qdcDensityLocal(int reference_SiPM, TClonesArray* scifi_hits, int radius) {
        ...
        for (int i = 0; i < entries; ++i) {
            ...
            if (!validateHitLocal(hit, ref_station, orientation)) continue;
...
def process_single_file_worker(args_tuple):
    ...
    input_file, output_parquet, geofile, parfile, first_event, max_events = args_tuple
    t0 = time.time()

    f_in = ROOT.TFile.Open(input_file, "READ")
    if not f_in or f_in.IsZombie():
        print(f"Worker Error: Could not open {input_file}")
        return None
    ...
    for i_event in range(start_ev, end_ev):
        ...
        if hasattr(event, "EventHeader") and hasattr(event.EventHeader, "ClassName") and event.EventHeader.ClassName() == "SNDLHCEventHeader":
        ...
        if not has_precomputed_hough and muon_reco_task:
                ...
                for l_idx in range(1, 4):
                    ...
                    if slot < len(p_lines):
                        ...
                        if sf_m and len(indices) > 0:
                            l_qdcs = [sf_m.hit_qdc[i] for i in indices]
                            l_hit_w = [sf_m.hit_w_density[i] for i in indices]
                            l_qdc_w = [sf_m.qdc_w_density[i] for i in indices]
                            row[f"{proj}_sf_sum_qdc_{l_idx}"] = sum(l_qdcs)
                            row[f"{proj}_sf_max_qdc_{l_idx}"] = max(l_qdcs)
                            row[f"{proj}_sum_qdc_{l_idx}"] = sum(l_qdcs)
                            row[f"{proj}_max_qdc_{l_idx}"] = max(l_qdcs)
                            row[f"{proj}_sf_sum_hit_w_density_{l_idx}"] = sum(l_hit_w)
                            row[f"{proj}_sf_max_hit_w_density_{l_idx}"] = max(l_hit_w)
                            row[f"{proj}_sum_hit_w_density_{l_idx}"] = sum(l_hit_w)
                            row[f"{proj}_max_hit_w_density_{l_idx}"] = max(l_hit_w)
                            row[f"{proj}_sf_sum_qdc_w_density_{l_idx}"] = sum(l_qdc_w)
                            row[f"{proj}_sf_max_qdc_w_density_{l_idx}"] = max(l_qdc_w)
                            row[f"{proj}_sum_qdc_w_density_{l_idx}"] = sum(l_qdc_w)
                            row[f"{proj}_max_qdc_w_density_{l_idx}"] = max(l_qdc_w)
                    else:
    ...
    n_sig = df["is_signal"].sum() if "is_signal" in df else 0
    print(f"  -> Done '{os.path.basename(input_file)}': {len(df):,} events ({n_sig:,} signal, {len(df.columns)} features) in {elapsed:.2f}s -> '{os.path.basename(output_parquet)}'")
    return len(df), n_sig
...
def main():
    ...
    parser.add_argument("--merge", dest="merge_output", default="", help="Optional unified destination path to concatenate all output parquet files into one master dataset")
    ...
    for f in matched_files:
        ...
        if "%s" in args.output_pattern:
            ...
            out_p = f"{os.path.splitext(args.output_pattern)[0]}_{base}.parquet"
        ...
        tasks.append((f, out_p, assigned_geo, args.par_file, args.start_event, args.n_events))
    ...
    # Optional Unified Merge
    if args.merge_output and output_files:
        print(f"\nConcatenating {len(output_files)} Parquet files into '{args.merge_output}'...")
        dfs = [pd.read_parquet(f) for f in output_files if os.path.exists(f) and os.path.getsize(f) > 0]
        if dfs:
            master_df = pd.concat(dfs, ignore_index=True)
            os.makedirs(os.path.dirname(os.path.abspath(args.merge_output)), exist_ok=True)
            master_df.to_parquet(args.merge_output, engine="pyarrow", compression="snappy", index=False)
            print(f"  Master dataset saved: {len(master_df):,} rows ({master_df['is_signal'].sum():,} signal) | Size: {os.path.getsize(args.merge_output)/(1024*1024):.1f} MB")
            
            if args.clean_intermediate:
                print(f"Cleaning up {len(output_files)} intermediate parquet files...")
                for f in output_files:
                    if os.path.exists(f) and os.path.abspath(f) != os.path.abspath(args.merge_output):
                        try:
                            os.remove(f)
                        except OSError:
                            pass
